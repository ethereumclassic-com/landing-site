...
/**
 * Pool shares come from the server, derived from Blockscout block attribution.
 * They used to render from a hardcoded `hashShare` in data/mining.ts, which had
 * drifted badly — F2Pool 41% against a live 60%, and two pools listed that are
 * no longer mining ETC at all. Static pool metadata (fee, payout, scheme) is
 * still local; only the share is live.
 */
export default function MiningStatsClient({ livePools }: { livePools: LivePool[] }) {
  /**
   * The chain decides who is mining; data/mining.ts only adds fee and payout
   * detail. Driving the row list the other way round showed pools with zero
   * blocks and hid ones that are mining but have no local entry.
   * "Others" is the remainder, not a pool.
   */
  const displayedPools = livePools
    .filter((lp) => lp.name !== 'Others')
    .map((lp) => ({
      live: lp,
      meta: miningPools.find((m) => m.name.toLowerCase() === lp.name.toLowerCase()),
    }))
    ...
    <main className="min-h-screen bg-[var(--bg)] pt-24 pb-16">
      ...
      <section className="px-6 pb-12 md:px-10 lg:px-12">
        <div className="mx-auto max-w-6xl">
          <div
          >
            <HashRateChart pools={miningPools} livePools={livePools} />
          </div>
        </div>
      </section>
          ...
          >
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {displayedPools.map(({ live, meta }) => (
                    <tr key={live.name} className="border-b border-[var(--border)]/50 last:border-0">
                      <td className="py-3">
                        <div className="flex items-center gap-2">
                          <span className="font-medium text-[var(--text-primary)]">{live.name}</span>
                          {meta?.recommended && (
                      ...
                      <td className="py-3 text-center">
                        <span className="font-mono text-[var(--text-primary)]">{meta ? `${meta.fee}%` : '—'}</span>
                      </td>
                      <td className="py-3 text-center">
                        <span className="font-mono text-[var(--text-primary)]">{meta ? `${meta.minPayout} ETC` : '—'}</span>
                      </td>
                      <td className="py-3 text-right">
                        <span className="text-sm text-[var(--color-text-muted)]">
                          {meta ? meta.payoutScheme.join(', ') : '—'}
                        </span>
                      </td>
                    </tr>
                  ))}

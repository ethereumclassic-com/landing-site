  ...
  return (
    <main className="min-h-screen bg-[var(--bg)] pt-24 pb-16">
      ...
      <section className="px-6 pb-12 md:px-10 lg:px-12">
        <div className="mx-auto max-w-6xl">
          <motion.div initial="hidden" animate="visible" variants={staggerContainer}>
            ...
            <motion.div variants={fadeInUp}>
              <h1 className="text-3xl font-bold text-white md:text-4xl lg:text-5xl">Developer Directory</h1>
      ...
      <section className="px-6 pb-12 md:px-10 lg:px-12">
        <div className="mx-auto max-w-6xl">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <h2 className="mb-6 text-xl font-semibold text-white">Networks</h2>
              ...
              {networks.map((net) => (
                <div key={net.chainId} className="rounded-xl border border-[var(--border)] bg-[var(--panel)] p-5">
                  ...
                  <div className="mt-3 space-y-1 text-sm text-[var(--color-text-muted)]">
                    <div>Chain ID: <span className="font-mono text-white">{net.chainId}</span></div>
                    <div>Symbol: <span className="font-mono text-white">{net.symbol}</span></div>
        ...
        <div className="mx-auto max-w-6xl">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-xl font-semibold text-white">RPC Endpoints ({rpcEndpoints.length})</h2>
                  ...
                  <tbody className="divide-y divide-[var(--border)]">
                    {rpcEndpoints.map((rpc) => (
                      <tr key={rpc.url} className="hover:bg-[var(--bg)]/50">
                        <td className="px-4 py-3 font-medium text-white">{rpc.provider}</td>
      ...
      <section className="px-6 pb-12 md:px-10 lg:px-12">
        <div className="mx-auto max-w-6xl">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <h2 className="mb-6 text-xl font-semibold text-white">Node Clients ({nodeClients.length})</h2>
      ...
      <section className="px-6 pb-12 md:px-10 lg:px-12">
        <div className="mx-auto max-w-6xl">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <h2 className="mb-6 text-xl font-semibold text-white">Developer Tools ({devTools.length})</h2>
            <div className="grid gap-4 md:grid-cols-2">
              {devTools.map((tool) => (

...
function CategoryCard({ category, index }: { category: ArticleCategory; index: number }) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <svg
          className="h-5 w-5 text-[var(--color-text-muted)] transition-transform group-hover:translate-x-1 group-hover:text-[var(--color-primary)]"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          strokeWidth={2}
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {featuredArticles.map((article, index) => (
              <NewsCard key={article.slug} article={article} index={index} variant="featured" />
            ))}
          </div>
        </div>
      </section>
